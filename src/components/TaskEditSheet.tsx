...
interface TaskEditSheetProps {
  task: Task;
  onSave: (id: string, title: string, description: string, dueTime?: string, filter?: FilterType) => void;
  onClose: () => void;
}

export function TaskEditSheet({ task, onSave, onClose }: TaskEditSheetProps) {
  const [title, setTitle] = useState(task.title);
  const [description, setDescription] = useState(task.description);
  const [dueTime, setDueTime] = useState(task.dueTime || '');
  const [isLater, setIsLater] = useState(task.filter === 'later');

  const handleSubmit = () => {
    if (!title.trim()) return;
    onSave(task.id, title.trim(), description.trim(), dueTime || undefined, isLater ? 'later' : 'tomorrow');
    onClose();
  };
    ...
    >
      ...
      {isLater && (
        <p className="text-xs text-muted-foreground px-6 -mt-2 mb-2">
          ✓ This task will be moved to the Later list
        </p>
      )}
